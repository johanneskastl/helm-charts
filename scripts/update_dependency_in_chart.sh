...
[[ "${#}" == "4" ]] || {
    echo "Please give the chart name, the update type, the depName and the depName's new version as only arguments"
    exit 3
}
...
sed -i "/kind/ {n; s/description:.*/description: update dependency ${DEPNAME} to ${DEPNAME_NEW_VERSION}/g}" "charts/${CHART}/Chart.yaml"
...
* update dependency ${DEPNAME} to ${DEPNAME_NEW_VERSION}
